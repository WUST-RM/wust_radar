...
#     command = ["/home/hy/TensorRT-8.5.2.2/bin/trtexec",
#                "--onnx=" + onnx_path,
...
def run_trtexec(onnx_path, save_engine_path, min_batch, opt_batch, max_batch, input_name, shape, fp16=False):
    # 确保保存目录存在
    os.makedirs(os.path.dirname(save_engine_path), exist_ok=True)

    command = [
        "/home/hy/TensorRT-10.6.0.26/bin/trtexec",
        f"--onnx={onnx_path}",
        f"--saveEngine={save_engine_path}",
        f"--minShapes={input_name}:{min_batch}x3x{shape}",
        f"--optShapes={input_name}:{opt_batch}x3x{shape}",
        f"--maxShapes={input_name}:{max_batch}x3x{shape}"
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--saveEngine", required=True, help="Path to save the TensorRT engine file.")
    parser.add_argument("--Shape", default="224x224", help="Input shape, e.g., 224x224")
    parser.add_argument("--minBatch", default="1", help="Minimum batch size for input.")
    parser.add_argument("--optBatch", default="10", help="Optimum batch size for input.")
    parser.add_argument("--maxBatch", default="20", help="Maximum batch size for input.")
    parser.add_argument("--input_name", default="input", help="Name of the input tensor.")
    ...
    run_trtexec(
        args.onnx, args.saveEngine, args.minBatch, args.optBatch, args.maxBatch,
        args.input_name, args.Shape
    )